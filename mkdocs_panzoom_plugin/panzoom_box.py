...
def create_info_box(soup, config):
    ...
    if always_hint:
        info_box = soup.new_tag("div", **{"class": css_class})
    else:
        info_box = soup.new_tag("div", **{"class": css_class + " panzoom-hidden"})
    ...
    info_box.string = key_messages.get(
        key, f"Press modifier key{action_text}"
...
def create_button_info(soup):
    ...
    info = soup.new_tag("button", **{"class": "panzoom-info panzoom-button"})
...
def create_button_reset(soup):
    ...
    reset = soup.new_tag("button", **{"class": "panzoom-reset panzoom-button"})
...
def create_button_max(soup):
    ...
    max = soup.new_tag("button", **{"class": "panzoom-max panzoom-button"})
...
def create_button_min(soup, hidden=True):
    ...
    if hidden:
        min = soup.new_tag("button", **{"class": "panzoom-min panzoom-button panzoom-hidden"})
    else:
        min = soup.new_tag("button", **{"class": "panzoom-min panzoom-button"})
...
def create_button_zoom_in(soup):
    ...
    zoom_in = soup.new_tag("button", **{"class": "panzoom-zoom-in panzoom-button"})
...
def create_button_zoom_out(soup):
    ...
    zoom_out = soup.new_tag("button", **{"class": "panzoom-zoom-out panzoom-button"})
...
def create_panzoom_box(soup, config, id):
    ...
    panzoom_box = soup.new_tag(
        "div",
        **{"class": "panzoom-box", "id": "panzoom" + str(id), "oncontextmenu": "return false;"},
    )
...
def create_fullscreen_modal(soup, config):
    ...
    modal = soup.new_tag(
        "div", **{"class": "panzoom-fullscreen-modal", "id": "panzoom-fullscreen-modal"}
    )
